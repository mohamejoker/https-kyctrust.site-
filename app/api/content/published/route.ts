...
export async function POST(req: Request) {
  ...
  const rl = await rateLimit(`content:publish:${ip}`, 30, 300) // 30 requests per 5 minutes
  if (!rl.ok) {
  ...
  await setSetting("published_content", data)
